            ...
            <main className="relative pt-32 pb-20 px-6 max-w-7xl mx-auto">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 md:grid-rows-2 gap-4 auto-rows-[minmax(0,1fr)]">
                    ...
                    <BentoCard className="md:col-span-2 md:row-span-1 min-h-[300px]" delay={0.1}>
                        <div className="mb-6">
                            <div className="w-12 h-12 rounded-2xl bg-blue-500/10 dark:bg-blue-500/20 text-blue-600 dark:text-blue-400 flex items-center justify-center text-xl mb-4 group-hover:scale-110 transition-transform">
                                <FaPaperPlane />
                            </div>
                            <h3 className="text-3xl font-bold text-zinc-900 dark:text-white mb-2">CV de Alto Impacto</h3>
                            <p className="text-zinc-600 dark:text-zinc-400">Diseño estratégico optimizado para ATS. No es solo un documento, es tu pase a la entrevista.</p>
                        </div>
                        <div className="flex items-center justify-between mt-auto pt-6 border-t border-zinc-100 dark:border-white/5">
                            <ul className="flex gap-4 text-xs text-zinc-500 font-mono">
                                <li>PDF + EDITABLE</li>
                                <li>REDACCIÓN PERSUASIVA</li>
                            </ul>
                            <span className="text-xl font-bold text-zinc-900 dark:text-white">$50 USD</span>
                        </div>
                    </BentoCard>

                    {/* Card 2: Web Portfolio - Tall */}
                    <BentoCard className="md:col-span-1 md:row-span-2 bg-white dark:bg-gradient-to-b dark:from-zinc-900/80 dark:to-black border-purple-500/20" delay={0.2}>
                        <div className="mb-6">
                            <div className="w-12 h-12 rounded-2xl bg-purple-500/10 dark:bg-purple-500/20 text-purple-600 dark:text-purple-400 flex items-center justify-center text-xl mb-4 group-hover:scale-110 transition-transform">
                                <FaCode />
                            </div>
                            <h3 className="text-3xl font-bold text-zinc-900 dark:text-white mb-2">Portafolio Web</h3>
                            <p className="text-zinc-600 dark:text-zinc-400 text-sm mb-6">Diferénciate del 99% de los candidatos con un sitio web personal interactivo.</p>

                            <div className="space-y-3">
                                <div className="p-3 rounded-xl bg-zinc-50 dark:bg-zinc-800/50 border border-zinc-100 dark:border-white/5 text-xs text-zinc-700 dark:text-zinc-300">
                                    <FaCheck className="inline mr-2 text-purple-500 dark:text-purple-400" /> Dominio Propio (.com)
                                </div>
                                <div className="p-3 rounded-xl bg-zinc-50 dark:bg-zinc-800/50 border border-zinc-100 dark:border-white/5 text-xs text-zinc-700 dark:text-zinc-300">
                                    <FaCheck className="inline mr-2 text-purple-500 dark:text-purple-400" /> Diseño Único
                                </div>
                                <div className="p-3 rounded-xl bg-zinc-50 dark:bg-zinc-800/50 border border-zinc-100 dark:border-white/5 text-xs text-zinc-700 dark:text-zinc-300">
                                    <FaCheck className="inline mr-2 text-purple-500 dark:text-purple-400" /> Modo Oscuro/Claro
                                </div>
                            </div>
                        </div>
                        <div className="mt-auto">
                            <p className="text-xs text-center text-zinc-500 mb-2">INVERSIÓN DESDE</p>
                            <div className="text-3xl font-black text-center text-transparent bg-clip-text bg-gradient-to-r from-purple-600 to-pink-600 dark:from-purple-400 dark:to-pink-400">
                                $200 USD
                            </div>
                        </div>
                    </BentoCard>
                    ...
                    <BentoCard className="md:col-span-1 min-h-[250px]" delay={0.3}>
                        <div className="flex flex-col h-full items-center justify-center text-center">
                            <div className="flex -space-x-2 mb-4">
                                {[1, 2, 3].map(i => (
                                    <div key={i} className="w-10 h-10 rounded-full border-2 border-white dark:border-zinc-950 bg-zinc-100 dark:bg-zinc-800 flex items-center justify-center text-xs text-zinc-500">
                                        User
                                    </div>
                                ))}
                            </div>
                            <p className="text-lg font-medium text-zinc-900 dark:text-white">"Conseguí empleo en 2 semanas"</p>
                    ...
                    {/* Card 4: LinkedIn */}
                    <BentoCard className="md:col-span-1 bg-zinc-50 dark:bg-zinc-900" delay={0.4}>
                        <FaFingerprint className="text-4xl text-emerald-600 dark:text-emerald-500 mb-6 opacity-80" />
                        <h3 className="text-2xl font-bold text-zinc-900 dark:text-white">LinkedIn Makeover</h3>
                        <p className="text-sm text-zinc-600 dark:text-zinc-400 mt-2">Optimización de perfil para aparecer en búsquedas.</p>
                        <div className="mt-4 pt-4 border-t border-zinc-200 dark:border-white/5 flex justify-between items-center">
                            <span className="text-xs text-emerald-600 dark:text-emerald-400 font-mono">RANKING #1</span>
                            <span className="font-bold text-zinc-900 dark:text-white">$40 USD</span>
                        </div>
                    </BentoCard>
